...
public class EventAPI {
	...
	public ResponseEntity<?> addEvent(@RequestBody Event newEvent, UriComponentsBuilder uri) {
		if (newEvent.getId() != 0 || hasNullElements(newEvent)) {
	...
	public ResponseEntity<?> putEvent(@RequestBody Event newEvent, @PathVariable("eventId") long eventId) {
		if (newEvent.getId() != eventId || hasNullElements(newEvent)) {
			return ResponseEntity.badRequest().build();
		}
		newEvent = repo.save(newEvent);
	...
	private boolean hasNullElements(Event event) {
		return event.getCode() == null || event.getTitle() == null || event.getDescription() == null;
	}
